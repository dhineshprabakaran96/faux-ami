...
def get_average_mfccs(directory, num_frames=None):
    """Calculates the average MFCCs from multiple audio files, handling varying lengths."""
    mfccs_list = []
    for file_path in glob.glob(os.path.join(directory, "*.mp3")):
        mfccs = extract_features(file_path)
        if mfccs is not None:
            if num_frames is not None:
                mfccs = mfccs[:num_frames] # Truncate or pad to uniform length.
